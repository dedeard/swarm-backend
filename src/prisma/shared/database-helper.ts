...
export class DatabaseHelper {
  ...
  static async isUserInCompany(userId: string, companyId: string): Promise<boolean> {
    try {
      const userCompany = await this.basePrismaClient.$executeRaw`
  ...
  static async isCompanyAdmin(userId: string, companyId: string): Promise<boolean> {
    try {
      const adminRole = await this.basePrismaClient.$executeRaw`
  ...
  static async hasPermission(userId: string, permission: string): Promise<boolean> {
    try {
      ...
      const result = await clientToUse.$executeRaw`
      ...
      // $executeRaw with COUNT returns a number, not an array
   ...
   */
  static async getAgentOwner(agentId: string): Promise<{ user_id: string } | null> {
    try {
      const result = await this.basePrismaClient.$queryRaw`
        ...
        LIMIT 1
      `
      return Array.isArray(result) && result.length > 0 ? (result[0] as { user_id: string }) : null
    } catch (error) {
   ...
   */
  static async getAgentCompany(agentId: string): Promise<{ company_id: string | null; user_id: string } | null> {
    try {
      const result = await this.basePrismaClient.$queryRaw`
        ...
        LIMIT 1
      `
      return Array.isArray(result) && result.length > 0 ? (result[0] as { company_id: string | null; user_id: string }) : null
    } catch (error) {
  ...
  static async getUserAdminCompanies(userId: string): Promise<string[]> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
        SELECT uc.company_id FROM "swarm-company".user_companies uc
        JOIN "swarm-rbac".roles r ON uc.role_id = r.role_id
        WHERE uc.user_id = ${userId}::UUID
        AND r.role_name IN ('admin', 'owner', 'swarm_company_admin')
      `
      return Array.isArray(result) ? result.map((row: any) => row.company_id) : []
    } catch (error) {
   ...
   */
  static async getUserRoles(userId: string, companyId?: string): Promise<string[]> {
    try {
      let query
      if (companyId) {
        query = this.basePrismaClient.$executeRaw`
      ...
      return Array.isArray(result) ? result.map((row: any) => row.role_name) : []
    } catch (error) {
   ...
   */
  static async canAccessAgent(userId: string, agentId: string, userRole: string): Promise<boolean> {
    try {
      ...
      const publicCheck = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async getAgentsByCompany(companyId: string, userId: string): Promise<any[]> {
    try {
   ...
   */
  static async addUserToCompany(userId: string, companyId: string, roleId?: string): Promise<void> {
    try {
      if (roleId) {
   ...
   */
  static async canAccessComponent(userId: string, componentId: string, userRole: string): Promise<boolean> {
    try {
   ...
   */
  static async getComponentsByCompany(companyId: string, userId: string): Promise<any[]> {
    try {
      // First check if user has access to this company
      const hasAccess = await this.isUserInCompany(userId, companyId)
      if (!hasAccess) return []
   ...
   */
  static async canAccessUserProfile(userId: string, targetUserId: string, userRole: string): Promise<boolean> {
    try {
   ...
   */
  static async checkOrganizationNameExists(organizationName: string, userId: string): Promise<boolean> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async getOrganizationById(organizationId: string): Promise<any | null> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async canAccessOrganization(userId: string, organizationId: string, userRole: string): Promise<boolean> {
    try {
   ...
   */
  static async getOrganizationsByCompany(companyId: string, userId: string): Promise<any[]> {
    try {
      // First check if user has access to this company
      const hasAccess = await this.isUserInCompany(userId, companyId)
      if (!hasAccess) return []
   ...
   */
  static async checkTeamNameExists(teamName: string, userId: string): Promise<boolean> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async canAccessTeam(userId: string, teamId: string, userRole: string): Promise<boolean> {
    try {
   ...
   */
  static async getTeamsByCompany(companyId: string, userId: string): Promise<any[]> {
    try {
      // First check if user has access to this company
      const hasAccess = await this.isUserInCompany(userId, companyId)
      if (!hasAccess) return []
   ...
   * Add agent to team
   */
  static async addAgentToTeam(teamId: string, agentId: string): Promise<boolean> {
    try {
      await this.basePrismaClient.$executeRaw`
   ...
   */
  static async removeAgentFromTeam(teamId: string, agentId: string): Promise<boolean> {
    try {
      await this.basePrismaClient.$executeRaw`
   ...
   */
  static async isAgentInTeam(teamId: string, agentId: string): Promise<boolean> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async checkLlmstxtNameExists(name: string, userId: string): Promise<boolean> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async canAccessLlmstxt(userId: string, llmstxtId: string, userRole: string): Promise<boolean> {
    try {
   ...
   */
  static async getLlmstxtByCompany(companyId: string, userId: string): Promise<any[]> {
    try {
      // First check if user has access to this company
      const hasAccess = await this.isUserInCompany(userId, companyId)
      if (!hasAccess) return []
   ...
   */
  static async removeLlmstxtFromAgent(agentId: string, llmstxtId: string): Promise<boolean> {
    try {
      await this.basePrismaClient.$executeRaw`
   ...
   */
  static async isLlmstxtAssociatedWithAgent(agentId: string, llmstxtId: string): Promise<boolean> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async getWaitlistEntryById(waitlistEntryId: string): Promise<any | null> {
    try {
      const result = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async canAccessWaitlistEntry(userId: string, waitlistEntryId: string, userRole: string): Promise<boolean> {
    try {
  ...
  static async getMaxWaitlistPosition(): Promise<number> {
    try {
      ...
      const maxPosition = Array.isArray(result) && result.length > 0 ? result[0].max_position : 0
  ...
  static async getWaitlistEntryCount(): Promise<number> {
    try {
      ...
      const totalCount = Array.isArray(result) && result.length > 0 ? Number(result[0].total_count) : 0
      return totalCount
    } catch (error) {
   ...
   */
  static async updateWaitlistPositionsAfterApproval(approvedEntryId: string): Promise<any[]> {
    try {
      // Get the approved entry's position
      const approvedResult = await this.basePrismaClient.$executeRaw`
        SELECT position FROM "swarm-waitlist".waitlist_entries
        WHERE waitlist_entry_id = ${approvedEntryId}::UUID
      `

      if (!Array.isArray(approvedResult) || approvedResult.length === 0) return []
      const approvedPosition = approvedResult[0].position

      // Get entries to update
      const entriesToUpdate = await this.basePrismaClient.$executeRaw`
   ...
   */
  static async updateWaitlistEntryPosition(entryId: string, newPosition: number): Promise<boolean> {
    try {
      await this.basePrismaClient.$executeRaw`
  ...
  static async getMaxWaitlistQuestionOrder(): Promise<number> {
    try {
      ...
      const maxOrder = Array.isArray(result) && result.length > 0 ? result[0].max_order : 0
      return maxOrder || 0
    } catch (error) {
